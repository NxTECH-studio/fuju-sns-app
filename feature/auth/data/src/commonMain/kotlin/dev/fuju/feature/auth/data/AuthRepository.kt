...
class AuthRepository(
    ...
    override suspend fun logout() {
        runCatching {
            client.post("/v1/auth/logout").throwIfErrorOrDiscard()
        }
    }

    override suspend fun refresh(): RefreshResponse =
        wrap {
            val res: RefreshResponseDto =
                client
    ...
    override suspend fun loadProfile(): User =
        wrap {
            val res: UserProfileResponseDto =
                client
                    .get("/v1/user/profile")
                    .also { it.throwIfError() }
                    .body()
    ...
    override suspend fun setupMFA(): MFASetupResult =
        wrap {
            val res: MFARegisterResponseDto =
                client
                    .post("/v1/auth/mfa/register")
                    .also { it.throwIfError() }
                    .body()
            MFASetupResult(res.secret, res.qrCode, res.recoveryCodes)
        }

    override suspend fun enableMFA(code: String): User =
        wrap {
            client
    ...
    override suspend fun disableMFA(code: String): User =
        wrap {
            client
    ...
    override suspend fun socialCallback(
        ...
        wrap {
            val res: VerifyMFAResponseDto =
                client
                    .post("/v1/auth/callback/${provider.slug}") {
                        contentType(ContentType.Application.Json)
    ...
    suspend fun disconnectSocial(
        ...
        wrap {
            client
                .delete("/v1/auth/disconnect/${provider.slug}/$providerUserId")
                .throwIfErrorOrDiscard()
            loadProfile()
        }

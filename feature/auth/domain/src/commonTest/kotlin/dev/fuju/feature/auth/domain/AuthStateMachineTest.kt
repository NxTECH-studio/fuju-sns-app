...
class AuthStateMachineTest {
    ...
    @Test
    fun bootstrapTransitionsToAuthenticated() =
    ...
    @Test
    fun bootstrapWith401MovesToUnauthenticated() =
        runTest(StandardTestDispatcher()) {
            val actions =
                FakeAuthActions(
                    refreshBehavior = {
    ...
    @Test
    fun loginRequiresMFAWhenFlagged() =
        runTest(StandardTestDispatcher()) {
            val actions =
                FakeAuthActions(
                    loginBehavior = { _, _ ->
    ...
    @Test
    fun verifyMFAFailureIncrementsAttempts() =
        runTest(StandardTestDispatcher()) {
            val actions =
                FakeAuthActions(
    ...
    @Test
    fun logoutReturnsToUnauthenticated() =
        runTest(StandardTestDispatcher()) {
            val actions = FakeAuthActions()
            val machine = newMachine(actions, scope = this)
            machine.login("alice", "pw")
    ...
    @Test
    fun stateFlowEmitsStatusSequence() =
        runTest(StandardTestDispatcher()) {
            val actions = FakeAuthActions()
            val machine = newMachine(actions, scope = this)
            machine.state.test {
